...
/*   Updated: 2024/08/20 15:06:54 by sabras           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo	*ft_init_philos(t_data *data)
{
	...
	if (!philos)
		return (ft_print_error("failed to allocate memory for philos"), NULL);
	i = 0;
	while (i < data->nb_philos)
	{
		...
		if (pthread_mutex_init(philos[i].fork_r, NULL) != 0)
			return (ft_print_error("failed to init mutex"), NULL);
...
int	ft_create_threads(t_data *data, t_philo *philos)
{
	...
	while (i < data->nb_philos)
	{
		if (pthread_create(&philos[i].thread, NULL, routine, &philos[i]) != 0)
			return (ft_print_error("failed to create thread"), 0);
		i++;
	}
	i = 0;
	while (i < data->nb_philos)
	{
		if (pthread_join(philos[i].thread, NULL) != 0)
			return (ft_print_error("failed to join thread"), 0);
		i++;
	}
	return (1);
}
